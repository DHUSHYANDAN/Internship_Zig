...
exports.checkAndFetchSSL = async (req, res) => {
  const { url } = req.body;
  ...
  try {
    ...
    // If certificate exists, check expiry date
    if (existingData) {
      const expiryDate = new Date(existingData.validTo);
      const today = new Date();

      if (expiryDate > today) {
        return res.status(200).json({ message: "Using stored data", data: existingData });
      }
    }

    // Fetch new SSL details
    ...
    socket.on("error", (err) => res.status(500).json({ message: "Socket error", error: err.message }));
  } catch (error) {
    res.status(500).json({ message: "Internal server error", error: error.message });
  }
};

exports.storeManagerAndSendMail = async (req, res) => {
